...
- For normal kits, there is only one main beam extrusion, so only six supports will be used.
...
- For XL kits, there are two main beam extrusions, so 12 supports will be used, and they should be positioned on the same side of the extrusions as the gantry joining bracket.
